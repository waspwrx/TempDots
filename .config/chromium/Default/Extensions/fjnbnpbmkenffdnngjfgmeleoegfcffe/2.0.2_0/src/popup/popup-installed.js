            ...
            sif.getStyleInfoByUrl(style.url)
                .then(function (styleInfo) {
                    return new Promise(function (resolve) {
                        ...
                        if (styleInfo.screenshot == null) {
                            imageExists("https://userstyles.org/style_screenshots/" + styleId + "_after.jpeg")
                                .then(function (status) {
                                    if (!status) {
                                        imageExists("https://userstyles.org/style_screenshots/" + styleId + "_after.png")
                                            .then(function (status) {
                                                if (!status) {
                                                    styleInfo.screenshot = IMAGE_URL_DEFAULT;
                                                } else {
                                                    styleInfo.screenshot = "https://userstyles.org/style_screenshots/" + styleId + "_after.png";
                                                }
                                                Object.assign(style, styleInfo);
                                                resolve(style);
                                            });
                                    } else {
                                        styleInfo.screenshot = "https://userstyles.org/style_screenshots/" + styleId + "_after.jpeg";
                                        Object.assign(style, styleInfo);
                                        resolve(style);
                                    }
                                });
...
function onFeedbackStyleClick(style) {
    return function (e) {
        analyticsEvent("Installed_styles_menu", "send feedback", style.styleId, () => { window.close(); });
    };
}

function onEditStyleClick(style) {
    return function (e) {
        analyticsEvent("Installed_styles_menu", "edit", style.styleId, () => { window.close(); });
    };
}
